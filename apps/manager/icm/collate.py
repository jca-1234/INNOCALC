...
from .snapshots import attachment_path, load_snapshot, package_body
...
def build_html(library: Library, registry: Any, selection: list[str], meta: dict[str, Any],
    ...
    for position, entry in enumerate(entries, start=1):
        ...
        if not revision.get("snapshotPath"):
            source = library.pdf_of(record)
            if source is None:
                raise ValueError(f"{entry['title']}: this legacy revision has no snapshot or filed PDF. "
                                 "Open, review and explicitly save a new revision before export.")
            count = pdf_tools.page_count(source)
            if not count:
                raise ValueError(f"Cannot read the saved PDF for {entry['title']}")
            entry.update({"imported": True, "sheets": count})
            inserts.append({"path": str(source), "pages": "", "after": body_page})
            sheet_number += count
            continue
        snapshot = load_snapshot(library.root, revision)
        entry.update({key: snapshot["summary"].get(key, entry.get(key))
                      for key in ("status", "worstUtil", "criticalCheck")})
        anchor = f"calc-{position:03d}"
        entry["anchor"] = anchor
        result = snapshot.get("result") or {}
        body, printed = package_body(snapshot["reportHtml"], anchor)
        sheets.append(body)
        body_page += printed
        entry["sheets"] = printed
        for cell in result.get("attachments") or []:
            saved_path = attachment_path(library.root, cell)
            pages = pdf_tools.parse_pages(cell.get("pages"), pdf_tools.page_count(saved_path))
            inserts.append({"path": str(saved_path), "pages": cell.get("pages", ""),
