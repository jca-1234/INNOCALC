...
function drawCells(list) {
  ...
  state.cells.forEach((cell, index) => {
    ...
    if (cell.type === "calc" && state.schema.liveEditing) {
      const hint = document.createElement("div");
      hint.className = "pad-meta pad-hint";
      hint.textContent = "Written in the sheet on the right";
      node.appendChild(hint);
    } else if (cell.type === "calc" || cell.type === "text") {
...
function showLearning() {
  ...
  const libraries = ((state.schema.libraries || {}).packages || []).map(item =>
    `<li><b>${esc(item.package)}</b> ${item.installed ? esc(item.version || "installed") : "not installed"}
     <span>${esc(item.purpose)}</span></li>`).join("");
  $("learningBody").innerHTML = `
    ...
    (<code>M_x</code>), and write units where you would on paper: <code>w = 5 kN/m</code>.
    A trailing <code>#&nbsp;kNm</code> comment converts the result into that unit, and a sum
    that does not balance dimensionally is reported instead of printed.</p>
    <p>Available functions: <code>${esc((state.schema.functions || []).join(", "))}</code></p>
    ${libraries ? `<h3>Libraries</h3><ul class="learning">${libraries}</ul>` : ""}
...
async function calculate() {
  ...
  try {
    ...
    const caret = capturePadCaret();
    $("reportPane").innerHTML = data.reportHtml;
    bindPadSheet(caret);
    renderDeclared((data.result && data.result.declared) || []);
...
/* ------------------------------------------- calculations typed in the sheet
   The pad renders its calculation cells as editable blocks, so an engineer
   writes the sum where it is printed rather than in a box on the left. The
   caret is carried across the recalculation that each keystroke triggers. */
function capturePadCaret() {
  const node = document.activeElement;
  if (!node || !node.dataset || node.dataset.padSource === undefined) return null;
  return { id: node.dataset.padSource, start: node.selectionStart,
    end: node.selectionEnd, scroll: node.scrollTop };
}
function padCell(id) {
...
function bindPadSheet(caret) {
  const pane = $("reportPane");
  if (!state.schema || state.schema.editor !== "cells") return;
  pane.querySelectorAll("[data-pad-source]").forEach(area => {
    const cell = padCell(area.dataset.padSource);
    if (!cell) return;
    fitToText(area);
  ...
  pane.querySelectorAll("[data-pad-title]").forEach(field => {
    const cell = padCell(field.dataset.padTitle);
    if (cell) field.addEventListener("input", () => { cell.title = field.value; onInputChanged(); });
  });
  pane.querySelectorAll("[data-pad-add]").forEach(button =>
    button.onclick = () => addCalculationCell(button.dataset.padAdd));
  pane.querySelectorAll("[data-pad-delete]").forEach(button =>
    button.onclick = () => removeCell(button.dataset.padDelete));
  if (!caret) return;
  const area = pane.querySelector(`[data-pad-source="${cssValue(caret.id)}"]`);
  if (!area) return;
  area.focus();
  try { area.setSelectionRange(caret.start, caret.end); } catch (error) { /* value changed */ }
  area.scrollTop = caret.scroll;
}
function cssValue(text) {
  return window.CSS && CSS.escape ? CSS.escape(text) : String(text).replace(/"/g, '\\"');
}
function addCalculationCell(afterId) {
  const cell = { id: `c${Date.now().toString(36)}`, type: "calc", title: "", source: "" };
  const at = state.cells.findIndex(item => item.id === afterId);
  state.cells.splice(at < 0 ? state.cells.length : at + 1, 0, cell);
  renderCellEditor(state.cells);
  onInputChanged();
}
function removeCell(id) {
  const at = state.cells.findIndex(item => item.id === id);
  if (at < 0) return;
...
/* ------------------------------------------------------ declared variables
   A pad line that is only a number is an input, not a result. Those collect in
   the panel on the left where they can be retyped without hunting through the
   sheet; everything derived from them stays where it was written. */
function renderDeclared(items) {
  const host = $("schemaGroups");
  let panel = document.getElementById("padDeclared");
  if (!state.schema || state.schema.editor !== "cells" || !items.length) {
    if (panel) panel.remove();
    return;
  }
  const signature = items.map(item => `${item.cellId}:${item.line}:${item.name}:${item.unit}`).join("|");
  if (panel && panel.dataset.signature === signature) {
    panel.querySelectorAll("[data-declared]").forEach(field => {
      const item = items[Number(field.dataset.declared)];
      if (item && document.activeElement !== field) field.value = item.literal;
    });
    return;
  }
  if (!panel) {
    panel = document.createElement("details");
    panel.id = "padDeclared";
    panel.open = true;
    const editor = document.getElementById("padEditor");
    host.insertBefore(panel, editor || null);
  }
  panel.dataset.signature = signature;
  panel.innerHTML = "<summary>Declared variables</summary>"
    + `<div class="pad-declared">${items.map((item, index) => `
      <label title="${esc(item.cellTitle || "")}">
        <span class="pad-declared-name">${item.symbol}</span>
        <input type="text" inputmode="decimal" data-declared="${index}" value="${esc(item.literal)}">
        <span class="pad-declared-unit">${esc(item.unit || "")}</span>
      </label>`).join("")}</div>`;
  panel.querySelectorAll("[data-declared]").forEach(field => {
    field.addEventListener("change", () => {
      const item = items[Number(field.dataset.declared)];
      const cell = padCell(item.cellId);
      if (!cell || !Number.isFinite(Number(field.value))) return;
      const lines = String(cell.source || "").split("\n");
      lines[item.line] = `${item.prefix}${field.value.trim()}${item.suffix}`;
