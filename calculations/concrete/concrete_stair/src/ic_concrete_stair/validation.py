"""Validation tool for the Concrete Stair Design module.
...
* ``baseline()``   - replays the retained Structural Toolkit CONCRETE STAIRS V5.06
...
SWEEP_BASE: dict[str, Any] = {
    "W": 1200, "cover": 20, "conc": 25, "density": 2400, "wsdl": 0.5,
    "loadtype": "Floor", "bar": 12, "fsy": 500, "ductilityClass": "N",
    "reoMode": "centres", "reoValue": 150, "Asc": 0, "dc": 20, "usefcmi": "N",
    "useVertM": "N", "useVertD": "Y", "deflectionBasis": "L", "k3": 1,
    "spanOverDeflection": 250, "spanOverDeflectionIncremental": 500,
}
SWEEP_GEOMETRY = [
    {"L": 3000, "th": 150, "riser": 175, "going": 250},
    {"L": 3800, "th": 175, "riser": 190, "going": 250},
    {"L": 5000, "th": 250, "riser": 190, "going": 275},
]
SWEEP_LOADS = [{"wll": 0.0}, {"wll": 4.0}, {"wll": 10.0}]
        ...
        for strength in SWEEP_STRENGTHS:
            for geometry in SWEEP_GEOMETRY:
                for loads in SWEEP_LOADS:
                    ...
                    geometry_out = result["geometry"]
                    if geometry_out["ath"] < geometry_out["th"]:
                        problems.append(f"{label}: the average thickness is below the throat")
                    if geometry_out["f"] < 1.0:
                        problems.append(f"{label}: the vertical thickness factor is below one")
                    bending = result["bending"]
                    if not 0.65 <= bending["phi"] <= 0.85:
                        problems.append(f"{label}: phi is outside its Table 2.2.2(b) bounds")
                    if result["deflection"]["kcs"] < engine.KCS_FLOOR - 1e-12:
                        problems.append(f"{label}: kcs fell below its Cl 8.5.3.2 floor")
                    if result["deflection"]["fdefIncremental"] > result["deflection"]["fdef"]:
                        problems.append(f"{label}: the incremental service load exceeds the "
                                        "total service load")
                    if result["loads"]["wdl"] <= 0:
                        problems.append(f"{label}: the self weight is not positive")
                    summary = headless.summarise(result)
                    if summary["status"] == "OK" and result["worstUtil"] > 1.0:
                        problems.append(f"{label}: an overloaded stair was reported OK")
...
def validate(cases: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "approval": "Independent engineering review of the CONCRETE STAIRS V5.06 transcription "
